...
To display a contour on a map by latitude and longitude, follow the steps in the walkthrough below.
...
. *_Optional:_* Drag the ‘wind’ field to the ‘Size’ region to weight the points by wind speed.
+
