...
. For a Dashboard, right-click the data block containing the measure  field (‘Total’) in the *Data Source* panel, and select ‘New Calculated Field’. For an Ad Hoc report chart, right-click the data source containing the measure  field (‘Total’) in the *Data Source* panel, and select ‘New Calculated Field’ from the menu.
+
image:commonscript:ChartExpDonut4.png[]
+
This opens the ‘Edit Calculated Field’ dialog box for a Dashboard or the Formula Editor for an Ad Hoc Report. (See xref:dataworksheet:CreateNewDataField.adoc[] for more information about Calculated Fields.)
