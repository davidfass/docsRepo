...
. *_Optional:_*  To populate a cell using script, press the ‘Edit’ button at the bottom of the dialog box. This opens the Script Editor and allows you to create a calculation based on cell values, parameter values, component values, or values from any accessible query or Data Worksheet.
+
...
You can insert this reference easily by selecting the field name from the ‘Cell’ folder in the editor. See xref:viewsheetscript:ReferenceTableData.adoc[] for more advanced cell reference syntax.
...
To reference data from other data blocks in the underlying Data Worksheet, see xref:viewsheetscript:ReferenceDataSourceData.adoc[]. To reference data from an arbitrary query or Data Worksheet, use `runQuery()` in the *onRefresh* handler. See xref:viewsheetscript:RunQueryFromScript.adoc[] for more information on using `runQuery()`.
****
