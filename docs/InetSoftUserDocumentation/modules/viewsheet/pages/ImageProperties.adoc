...
* *Scale*: The image is divided into nine regions, which scale independently. See xref:AddBackground.adoc[] for more information about 9-cell scaling.
...
Pop Component:: Select an individual component or group Container to display as a pop-up interface. When the user clicks the image, the selected component is displayed at the location of the image. Otherwise, the selected component is not visible in the Dashboard.
+
