...
. Select the *Content* tab, and set the export contents. Follow the steps below:
...
. Select the *Location* tab (if available). Choose ‘Desktop’ to save the exported file onto the local machine.  Choose ‘Repository’ and select a repository location to save the exported file onto the server.
+
TIP: Note: An ‘anonymous’ user cannot save to the repository, and a Snapshot cannot be saved to the repository.
+
When you export a Dashboard to the *Repository*, the Dashboard is displayed within the repository tree but does not pull live data from the database. The exported Dashboard contains archived data, reflecting the state of the Dashboard at the time that the export was made.
+
image:snapshotRepository.png[]
...
. On the *Content* tab, select the ‘Excel’ export option.
...
* xref:viewsheetscript:WriteDataToDatabase.adoc[] in xref:viewsheetscript:AdvancedDashboardScripting.adoc[] , for information on committing modified table data to a database.
====
