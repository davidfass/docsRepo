...
To create other formats, select the ‘Custom’ option and use the following date masks,  following the pattern presented in  xref:commonscript:DateFormatCodes.adoc[].
...
TIP: To insert a single-quote in the date format, use a double single-quote (e.g., `MMM ''yy`).
...
To create custom multipliers, see xref:administration:DefineCustomNumberFormats.adoc[] in xref:administration:ManageServer.adoc[].
...
The custom formats use a set of placeholders (`H`,`m`,`s`, etc.) to represent the duration. (See xref:commonscript:DateFormatCodes.adoc[]  for more information about the codes.) The ‘Pad with Zeros’ option places “0” in any position that has no value.
