...
You can attach hyperlinks  to the Chart graphical elements (e.g., the bars in a bar chart). To create hyperlinks on a Chart, follow the steps below:
...
[#HyperlinkDashboardReport]
=== Hyperlink to a Dashboard or Report

This example shows how Chart hyperlinks can be used to drill to the sample ‘OrderList’ report (in the _classes/inetsoft/demos_ directory).
...
+
NOTE: The ‘OrderList’ report is designed to accept five parameters. The ‘customer’ parameter represents the company name of the customer. You will configure the hyperlinks that you add to the ‘Customer’ > ‘Company’ attribute in the Dashboard to pass the company name into the ‘customer’ parameter of the ‘OrderList’ report.
...
. Deselect the ‘Self’ option, and enter the name “ReportFrame” in the ‘Target Frame’ field.
+
NOTE: The 'Self' setting is not applied during preview in Visual Composer, and is not available for hyperlinked Dashboards (i.e., Dashboards selected in the ‘Link’ menu). All hyperlinked Dashboards open in a separate browser tab.
+
This will cause the hyperlinked report to open in a window called “ReportFrame” (i.e., this is the browser DOM ‘name’ attribute). If you want the report to open in the original Dashboard frame, replacing the Dashboard, leave the ‘Self’ checkbox selected.
+
...
. Select the ‘customer’ parameter in the ‘Parameter Name’ menu. This is the name of the target parameter in the OrderList report.

. Select the ‘Company’ field from the ‘Value for Parameter’ menu. This is the name of the table column whose value should be passed into the report parameter when the user clicks the hyperlink.
+
...
. *_Optional:_*  Enable ‘Send Dashboard parameters’ to pass all existing Dashboard parameters to the target report or Dashboard. The Dashboard parameter names must exactly match those defined in the target report or Dashboard. See xref:viewsheetscript:parameter.adoc[] in xref:viewsheetscript:AdvancedDashboardScripting.adoc[] for more information about Dashboard parameters.

. *_Optional:_*  Select ‘Disable parameter prompt’ to suppress the parameter dialog box ordinarily produced by the drill-down report upon launch. (Prompting is automatically suppressed for parameters passed in the hyperlink itself.)

. *_Optional:_*  Select ‘Send selections as parameters’ to send the values of any Dashboard Filter components (Selection Lists, Range Sliders, etc.) to the target report or Dashboard. These values are passed into parameters in the target report or Dashboard that have the same name as the Filter component, as shown below:
+
...
For this example, clicking one of the hyperlinks will now launch the ‘OrderList’ report in a new browser window, and the ‘OrderList’ report will display _only_ the orders for the specific company the user clicked.
...
+
TIP: You can also specify a *table column* to provide the URL links by selecting the desired column from the ‘Link’ menu.
