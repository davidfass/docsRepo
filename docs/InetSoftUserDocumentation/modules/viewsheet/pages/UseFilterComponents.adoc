...
The following sections explain how to use filter components to dynamically explore your data.
...
If there is no {buttonApply} in the title bar, each selection you make is submitted immediately. To make multiple selections before submitting, hold the {buttonCtrl}  while making your selections.

include::viewsheet:partial$selectionIcons.adoc[]

The Selection List provides a *mini-toolbar* that allows you make convenient modifications to the current selection. Move the mouse over the Selection List to see the mini-toolbar. The following functions are available.

.Selection List Mini-Toolbar
****
...
{buttonChartShowEnlarged}:: Press to display the Selection List full screen. (Also available for Selection Tree and Range Slider.) Press the {buttonChartShowActualSize} to return the component to its original size.

{buttonSearch}:: Press to enter a search term in the text field. As you type, the Selection List updates to display only items having a match for your search text (appearing anywhere in the item). Press the {buttonClose} button in the text field to close the Search feature.
...
. *_Optional:_*  To switch the Calendar from single-page to double-page display, press the {buttonSwitchToRange}.
+
...
. *_Optional:_*  To switch the Calendar from displaying a single month to displaying the entire year, press the {buttonSwitchToYearView}.
+
...
+
In a ‘Month View’ calendar, click to select a day or week, or click the name of the month at the top of the calendar to select the entire month. In a ‘Year View’ calendar, click to select a month, or click the year at the top of the calendar to select the entire year.
...
. If the Calendar is double-page view, press the {buttonApply} to submit your selection. (If the Calendar is in single-page view, the selection is applied automatically.)

The range that you select is used to filter all data view and output components that are based on the same data block as the Calendar component.


== Bundle Filters in a Container
A user can add new selection components into an existing Selection Container if the ‘Allow User to Add/Remove Selections’ property is enabled. See xref:SelectionContainerProperties.adoc[] for more information.
...
[#AddSelectionContainerbyDraggingColumnfromTable]
=== Add Selection Container by Dragging Column from  Table
To add a new filter to a Selection Container, locate the column that you want to filter within an existing  Table or Crosstab. Drag the column into the Selection Container.String-type fields are added to the Selection Container as Selection Lists, while numerical-type and date-type fields are added to the Selection Container as Range Sliders.Method 2. Press the ‘Add Filter’ button: Press the  in the mini-toolbar of the Selection Container. This opens the ‘Add Filter’ dialog box, where you can select the field to add as a filter to the Selection Container.String-type fields are added to the Selection Container as Selection Lists, while numerical-type and date-type fields are added to the Selection Container as Range Sliders.Method 3. Add a post-aggregate filter: Add an ad hoc filter into the Selection Container by right-clicking on a field in a Chart or Table and selecting the ‘Filter’ option. See Use an ad hoc filter to filter post-aggerate crosstab data… and Use an ad hoc filter to filter post-aggregate chart data… for more information.String-type fields are added to the Selection Container as Selection Lists, while numerical-type and date-type fields are added to the Selection Container as Range Sliders.To remove a selection component from the Selection Container, right-click the component and select ‘Remove’. To collapse a selection component in a Selection Container, press the  in the selection component title bar. To expand a selection component in a Selection Container, press the . To clear the filters on all selection components in the Selection Container, press the  in the Selection Container title bar.
...
// copied from Add Filter Component... Selection Tree

Press the {buttonExpand} next to an item on the tree to expand the tree and view items at the next level in the hierarchy. To select all items under a given parent item in the tree, right-click the parent item and choose ‘Select Subtree’ from the context menu. {rightClickOptions} To clear all items under a given parent item in the tree, right-click the parent item and choose ‘Clear Subtree’ from the context menu.

image:selectSubtreeMenu.png[]


// cpied from Add Calendar


You can use a Calendar to select dates at the year, month, week, or day level. Note that the Calendar title bar indicates the selected range.

image:CalendarRangeTypes_1.png[]

To select a column of days (e.g., all Thursdays in a month), click the name of the desired day. (Requires that “day selection” be enabled for the Calendar.)

image:CalendarDaySelection.png[]

To select an entire year, click the name of the year.

image:CalendarYearSelection.png[]

The mini-toolbar in the title bar that allows you make convenient modifications to the calendar. (Move the mouse over the Calendar to see the mini-toolbar.)
...
. Select ‘Iowa’.
+
Note that this clears the ‘Northeast’ selection from the ‘Region’ list. In general. when you select an option from the ‘Others’ list, this will clear any incompatible selections that you previously have made in other Selections Lists. The divisions listed are now those compatible with the five currently selected states.
+
