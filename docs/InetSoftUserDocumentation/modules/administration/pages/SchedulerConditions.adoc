...
There are three types of conditions available: Time conditions, chained (completion) conditions, and user-defined conditions.  These are described below.

[NOTE]
====
The Scheduler condition ‘Start Time’ supports both 12-hour or 24-hour time values.  To set the time using a 12-hour (AM/PM) clock, use the xref:GeneralFormat.adoc[] page to enter a ‘Time Format’ string that ends with the `'a'` specification (e.g., `HH:mm:ss a`).  To set the time  using a 24-hour clock, enter a ‘Time Format’ that does *not* include the `'a'` specification (e.g., `HH:mm:ss`).
...
User Defined Condition:: To create a custom condition, follow the steps below:
+
****
. *Recommended*: Register the custom condition class, if you have not done so already. See xref:ConfigureScheduler.adoc[] for more information.
. Select ‘User Defined’ as the condition type under the Scheduler *Condition* tab.
. Select the desired condition  in the ‘Condition’ field. (If you did not register the condition class in the first step, type in the fully-qualified class name in the 'Condition Class' field.)
. Press the btn:[Save] button to save the condition.
****
