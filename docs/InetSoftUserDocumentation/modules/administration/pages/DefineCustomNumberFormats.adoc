...
You can define custom symbols and multipliers by adding the desired entries into the _userformat.xml_ file in the server repository (SREE Home).

include::ROOT:partial$SREEHome.adoc[]

For example, the following _userformat.xml_ file defines three new multipliers, “halfK”, “halfM”, and “halfB”, that divide the numerical values by 5x10^2^, 5x10^5^, and 5x10^8^, respectively.
...
When you restart the server with this _userformat.xml_ file in place, the new suffixes will be available for use with number format patterns, and will be displayed among the format presets for reports and Dashboards:
