...
Modular designs offer benefits in terms of reusability, and  simplify maintenance. When you use a Dashboard in multiple composite Dashboards, you can update all the composite Dashboards simultaneously by making changes centrally to the shared module.
...
The common Dashboard is now embedded within the host Dashboard, and all the components marked ‘Visible in External Dashboards’ within the common Dashboard are displayed. To synchronize selection and form components within two (or more) Dashboards, see below.
...
This feature is also available to end-users who view the host Dashboard in the Portal if there is at least one hidden element in the embedded Dashboard. (See xref:viewsheet:GeneralTab.adoc[] for more information about hiding components.)
...
NOTE: If you want to create a single filter component that filters multiple data blocks in a Data Worksheet, see xref:viewsheet:AddFilterComponents.adoc#FilterMultipleDataBlocks[Filter Multiple Data Blocks] in xref:viewsheet:AddFilterComponents.adoc[] instead.
