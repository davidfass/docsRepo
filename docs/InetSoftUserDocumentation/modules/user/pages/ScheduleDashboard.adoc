...
[#SetAlert]
== Set an Alert
An *alert* is a notification that a specified *data event* has occurred, for example, that total sales have fallen below some critical level. You can set an alert based on highlight conditions in a report or Dashboard, or based on Dashboard Gauge ranges.

To configure an alert for a report or Dashboard, follow the steps below:
...
. On the *Condition* tab, set the frequency with which the alert conditions should be tested. (See xref:#ScheduleConditions[] for more details on the available day and time conditions.) For example, if you want to test a report highlight condition every day, specify a ‘Daily’ condition.

. On the *Action* tab, specify the report or Dashboard that contains the alert condition you want to test.
+
image:alert1.png[]
...
The available actions  are described below. To specify multiple actions, press the btn:[Multiple Actions] button at the bottom right. This will display the schedule action list to which you can add, delete, or edit actions. To execute a scheduled task only when  certain data conditions are satisfied, see xref:#SetAlert[] on this page.

// COMMENTING OUT ALL REPORT ACTIONS
////
=== Report Action
The Report action executes a report and delivers or saves it. If the report requires parameter values, assign these parameters in the 'Creation Parameters' table at the bottom of the page. See xref:#CreationParameters[] below for more information about how to  configure parameters.


image:scheduleReport4.png[]


The following options are available for a 'Report' action:

.Enable Notification of Task Status
[%collapsible]
====
****
This option sends notifications to the email addresses that you specify in the 'Notify' field as a comma-separated list.  The email notification informs the recipient whether the report was successfully generated or not. Select ‘Notify only if failed’ to limit the notification only to the event of a task failure.
...
This option sends the report as an email attachment to the email addresses that you specify as a comma-separated list in the ‘To’ field. Use the 'Format' menu to choose the format in which to generate the report (PDF, Excel, etc.). For the ‘CSV’ format, use the ‘Delimiter’ option to specify the delimiter with which to separate individual values in the same row. Use the ‘Quote’ option to specify a character with which to bracket every value. Select ‘Keep Header’ to retain the table column header labels as the first row of the CSV file.

include::ROOT:partial$excelExportLimit.adoc[]

If the generated file is too large, it will be split into multiple emails. The ‘Bundled as zip’ option allows you to zip the attachment and optionally specify a password to encode the archive with WinZip 256-bit AES encryption. To embed the report itself within the email as HTML, select the ‘HTML (embed in email)’ option from the 'Format' menu. Note, in this case, that the appearance of the delivered report may not be fully preserved if the recipient’s email client is not standards-compliant.


Inserting Parameters:: The '`Subject`' line and '`Attachment Name`' can include parameters for automatic insertion of report name and time and date of generation. These parameters use the {JavaTextMessageFormat} syntax, where `{0}` represents the report  alias (or name, if no alias is specified), and `{1,date}` represents the time and date of report  generation. You can specify a format for the date parameter using the patterns described in xref:viewsheet:DataFormat.adoc[]. Some examples of automatically generated names are shown below.
+
|===
...
|===
For example, for a report name ‘SalesReport’, the ‘Subject’ specification `Report named {0}, generated at {1,time} on {1,date}` would produce  an email subject line similar to the following: `Report named SalesReport, generated at {localtime} on {localdate}`.
+
...
+
To include report  *creation parameter* values in the filename or subject line, simply place each parameter name within curly braces. The parameter names you enter  must exactly match the parameter names in the ‘Creation Parameters’ panel of the *Actions* tab. (See xref:#CreationParameters[] below.)  For example, if a parameter called `stateParam` is assigned a value of “NJ” in the ‘Creation Parameters’ panel, then the attachment name `pass:[myRep_{1,date,MMM-dd}_{stateParam}.pdf]` will yield a generated filename of `myRep_Nov-14_NJ.pdf`.
****
====

.Enable Save in Archive
[%collapsible]
====
****
This option saves the generated report in the report archive.  The archived copy is a snapshot of the report at the moment that the scheduled task executes, and the data is *not* subsequently updated. In order to archive a report, the archive must be properly configured by an administrator, and you must have proper permissions to save the report in the selected folder.

image:schedulerSaveArchive.png[]

The filename you provide in the ‘Name’ field can include parameters for automatic insertion of report name and time or date of report generation (see 'Inserting Parameters' below). Select a repository folder from the 'Folder' menu. To assign the default folder permissions to the saved report, select ‘Use folder permission’. To keep the report private, select ‘Grant all permissions to owner only’. Use the ‘Format’ menu to choose the format in which to generate the report (PDF, Excel, etc.).

*_Optional:_* You can set an *archive rule* to determine how long the archived report should persist in the repository. To set an archive rule, press the 'Edit Archive Rule' button to open the 'Archive Rule' dialog box. To set a fixed date after which the archived report should be deleted, choose 'Delete Report After', and press the 'Calendar' button to select the date. To set a lifespan for the archived report in the repository, select 'Delete Reports Older Than', and enter the desired range in years, months, and days.


image:scheduleReport5.png[]

IMPORTANT: If the administrator has configured the archive as a  file system archive, every time the scheduled task executes the new snapshot will replace the previously archived copy, regardless of the archive rule.


Inserting Parameters:: The filename can include parameters for automatic insertion of report name and time and date of generation. These parameters use the {JavaTextMessageFormat} syntax, where `{0}` represents the report  alias (or name, if no alias is specified), and `{1,date}` represents the time and date of report  generation. You can specify a format for the date parameter using the patterns described in xref:viewsheet:DataFormat.adoc[]. Some examples of automatically generated names are shown below.
+
|===
...
.Enable Save to Server
[%collapsible]
====
****
This option saves a copy of the report to the server file system or to an FTP server. Use the 'Format' menu to choose the format in which to generate the report ('PDF', 'PowerPoint', 'Excel', etc.)

image:schedulerSaveServer1.png[]
...
To save to an FTP server, enter a valid URL in the ‘Path’ field using the following format:

 (s)ftp://<username><password>@<hostname><port>/<filename>

Here `password` and `port` are optional. For SFTP connections, the known hosts file, _.ssh/known_hosts_, is automatically read from the user’s home directory. Before running the scheduled task, an administrator should populate the _known_hosts_ file by first connecting (SSH) to the target server from each InetSoft server node.
...
=== Dashboard Action
The Dashboard action executes a Dashboard and delivers or saves it. If the Dashboard requires parameter values, assign these parameters in the 'Creation Parameters' table at the bottom of the page. See xref:#CreationParameters[] below.


image:scheduleDashboard3.png[]

The following options are available for a 'Dashboard' action:
...
Insert Parameters:: The '`Subject`' line and '`Attachment Name`' can include parameters for automatic insertion of Dashboard name and time and date of generation. These parameters use the {JavaTextMessageFormat} syntax, where `{0}` represents the Dashboard  alias (or name, if no alias is specified), and `{1,date}` represents the time and date of Dashboard  generation. You can specify a format for the date parameter using the patterns described in xref:viewsheet:DataFormat.adoc[]. Some examples of automatically generated names are shown below.
+
|===
|Subject or Filename |Generated As...
...
====
****
This option saves a copy of the Dashboard to the server file system or to an FTP server. Use the ‘Format’ menu to choose the format in which to generate the Dashboard: ‘PDF’, ‘PowerPoint’, ‘Excel’, or ‘CSV’.
...
To save to an FTP server, enter a valid URL in the ‘Path’ field using the following format, where `password` and `port` are optional.
...
=== User Defined Action
The User Defined action allows you to use a custom Java action class, if one exists.
...
////
=== Burst Action
The Burst action delivers multiple versions of a specified report, tailoring each version to the particular recipient. A designer must must first configure the report for bursting in Style Studio. See Report Bursting in Advanced Report Design for more details.
////
