...
. To use a subquery in the condition, press the {buttonChangeValueType} and select ‘Subquery’. This displays the ‘Subquery’ button.
+
...
. Press ‘OK’ to exit the ‘Condition’ panel. The data blocks are now linked by a {buttonSubquery} to indicate that the ‘Fast Go Game1’ data block provides a subquery result to the ‘Wireless Mouse1’ data block.
