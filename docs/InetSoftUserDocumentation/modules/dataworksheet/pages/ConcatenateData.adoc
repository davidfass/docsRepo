...
[id=Union]
== Create a Union
By default, the union of two data blocks contains all of the unique rows from the two data blocks. The first data blocks you select is the data blocks whose distinct rows will appear first in the result. By default, no duplicate rows are retained in the union data blocks .

.Example: Union
****
Consider a query that returns a list of customers who have purchased more than 100 pieces of ‘Wireless Mouse’ and a second query returns a list of customers who have purchased more than 100 pieces of ‘Fast Go Game’. To see the contact information of all these customers  in a single data block, perform a union.
...
This creates a new data block, ‘Query1’.
+
NOTE: You can also drag the ‘Fast Go Game’ block immediately above or below the ‘Wireless Mouse’ block. Release when you see the ‘union’ symbol appear.
. Click the ‘Query1’ data block to select it. In the bottom panel, press the {buttonChangeView} and choose {buttonLiveDataView}.
+
...
Minus: The difference (minus) of two data blocks is the set of distinct rows found in one data block but not in the other. The first data block that you select is the data block whose unique rows are retained in the result.
...
[id=ChangeConcatenationType]
== Change the Concatenation Type
To change the concatenation type of a concatenated data block (intersection, union, minus), follow these steps:
