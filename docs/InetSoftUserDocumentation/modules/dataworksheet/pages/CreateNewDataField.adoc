...
+
image:dataworksheet:agileTransform3.gif[]
+
...
+
image:dataworksheet:agileTransform5.gif[]
+
...
+
image:dataworksheet:agileTransform6.gif[]
+
...
=====
ifdef::testdrive[]
. Edit the Dashboard to which you want to add an expression field. See [XXXX edit dash].
endif::[]
...
. In the *Toolbox* panel, right-click on the data block, data model, or query to which you want to add the calculated field. Select ‘New Calculated Field’ from the context menu.
+
ifdef::si[]
image:viewsheet:NewCalculatedField1.gif[]
endif::[]

ifdef::testdrive[]
image:testdrive:agileDash60.gif
endif::[]
+
This opens the ‘Calculated Field’ panel. #Note:# Your choice of 'Detail' or 'Aggregate' cannot be changed later.
+
...
. Select the ‘Detail’ option to perform calculations on the data directly provided by the data block, or click 'Aggregate' to perform calculations on the data after it has been aggregated (for example in a Chart or a Crosstab).
+
image:viewsheet:NewCalculatedField2.gif[]
+
...
+
.Read more about  detail-level calculation...
[%collapsible]
====
The available fields are listed in the top-left panel. Click on an available field in the panel to add it to the expression.

image:viewsheet:NewCalculatedField3.gif[]
...
+
[%collapsible]
.Read more about aggregate-level calculation…
====
...
[example]
=====
Consider an example: Your data set contains the fields ‘Sales Amount’ and ‘Order Number’, and you often want to display the average of the ‘Sales Amount’ field, i.e., ‘Average Sales Amount’. To make this aggregate quantity available to your components, create  ‘Average Sales Amount’ as an aggregate calculation with the following formula:
[source,javascript]
...
+
image:viewsheet:CalcFieldExample8.gif[]
+
...
+
image:viewsheet:CalcFieldExample9.gif[]
+
In the ‘Aggregate’ menu, select the desired aggregation method. Read more about the available methods below:
+
.Aggregation Methods
[example]
=====
...
+
image:viewsheet:CalcFieldExample10.gif[]
+
.. #Optional#  Press the ‘New Aggregate’ button again, and repeat the above steps to add as many additional aggregates as desired.
...
+
image:viewsheet:CalcFieldExample11.gif[]]
+
...
+
ifdef::si[]
image:viewsheet:NewCalculatedField4.gif[]
endif::[]
+
ifdef::testdrive[]
image:testdrive:agileDash61.gif[]
endif::[]

You can display this calculated field in Output Components (see xref:viewsheet:AddAKPI.adoc[]) or Data View Components (see xref:viewsheet:AddAChart.adoc[] and xref:viewsheet:AddTableOrCrosstab.adoc[]) in the same way that would display any other data field. #NOTE:# You cannot bind an aggregate-based calculated field to Filter Components.

.Explore More...
****
The sample
Global Visual Analysis Dashboard provides an example of using *calculated fields* to color the regions on a map.

To explore this sample Dashboard, download and import the  Global Visual Analysis Dashboard into your environment. (This requires access to Enterprise Manager.) See xref:administration:ImportAssets.adoc[] for instructions on how to import.
