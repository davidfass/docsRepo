...
+
image:agileData21_1.gif[]

.. To change an existing join, press the ‘Edit’ button next to the join.
+
image:agileData22_1.gif[]
.. To delete an existing join, press the ‘Delete’ button next to the join.
+
image:agileData23_1.gif[]

. #Optional:#  Press the btn:[Edit] button next to the ‘Conditions’ area to open the ‘Conditions’ dialog box. Use the menus to enter a desired filter condition and press btn:[Insert]. Then press btn:[OK].
+
image:agileData14_1.gif[]
+
NOTE: For full information about specifying conditions, see xref:FilterData.adoc[].


. #Optional:#  To make manual edits to the SQL string that will be sent to the database, press the btn:[Edit] button next to the ‘SQL’ area to open the ‘SQL’ panel for editing. Make the desired edits.
+
image:agileData15_1.gif[]
+
