= Access Data with an Expression
...
== Example: Reference Data from a Data Block
...
. Drag the following fields from the menu:TABLE[SA,PRODUCTS] table into an empty area in the right panel. (kbd:[Ctrl]-click the field names to select them all, and then drag them together.)
 ...
 CATEGORY_ID
+
image:dataworksheet:ReferenceCellExampleMakeTable6.gif[]
+
This creates a new data block named ‘PRODUCTS1’.
+
. Rename the ‘CATEGORY_ID’ column as ‘CAT_ID’. This will help to distinguish it from the data block you will add next. (See xref:dataworksheet:CreateNewWorksheet.adoc[] for instructions on how to rename a column.)
...
+
image:dataworksheet:ReferenceCellExampleMakeTableNoSQL.gif[]
+
...
+
image:dataworksheet:ReferenceCellExampleMakeTable7.gif[]
+
